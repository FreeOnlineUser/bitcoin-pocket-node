...
/**
 * Unit tests for shared P2P wire-format helpers (Wire).
 * The socket paths are exercised on-device; these cover the pure
...
class HeadersClientTest {
    ...
    fun `varint single byte`() {
        val (value, size) = Wire.readVarInt(byteArrayOf(0x7F), 0)
    ...
    @Test
    fun `varint zero`() {
        val (value, size) = Wire.readVarInt(byteArrayOf(0x00), 0)
    ...
    fun `varint uint16 - 2000 headers per message`() {
        ...
        val (value, size) = Wire.readVarInt(data, 0)
    ...
    fun `varint respects offset`() {
        ...
        val (value, size) = Wire.readVarInt(data, 2)
    ...
    fun `hexToInternal reverses byte order`() {
        ...
        val internal = Wire.hexToInternal(genesis)
    ...
    fun `hexToInternal round trips`() {
        ...
        val internal = Wire.hexToInternal(hex)
        assertEquals(hex, Wire.internalToHex(internal))
    }
}
